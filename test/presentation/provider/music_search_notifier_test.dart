...
import 'music_search_notifier_test.mocks.dart';

@GenerateMocks([SearchMusic])
void main() {
  late MusicSearchNotifier provider;
  late MockSearchMusic mockSearchMusic;
  late int listenerCallCount;

  setUp(() {
    listenerCallCount = 0;
    mockSearchMusic = MockSearchMusic();
    provider = MusicSearchNotifier(searchMusic: mockSearchMusic)
      ..addListener(() {
        listenerCallCount += 1;
      });
  });

  final tMusicModel = Music(
    wrapperType: "track",
    kind: "song",
    artistId: 909253,
  ...
  final tMovieList = <Music>[tMusicModel];
  const tQuery = 'avril';

  group('search movies', () {
    test('should change state to loading when usecase is called', () async {
      // arrange
      when(mockSearchMusic.execute(tQuery))
          .thenAnswer((_) async => Right(tMovieList));
      // act
      provider.fetchMusicSearch(tQuery);
      // assert
    ...
    test('should change search result data when data is gotten successfully',
      ...
      when(mockSearchMusic.execute(tQuery))
          .thenAnswer((_) async => Right(tMovieList));
      // act
      await provider.fetchMusicSearch(tQuery);
      // assert
